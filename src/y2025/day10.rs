...
fn part_a(input: String) -> usize {
    ...
    lines.par_iter().map(|line| {
        ...
        let buttons = parts[1..parts.len() - 1].iter().map(|part| {
            part[1..part.len() - 1]
                .split(",")
...
fn solve_b(target: Vec<usize>, buttons: Vec<Vec<usize>>) -> usize {
    ...
    for i in 0..target.len() {
        // a * pi + b * pi + ... = Pi
        let left_side = buttons.iter().enumerate()
            .map(|(j, button)| {
    ...
    let objective = button_clicks.iter()
