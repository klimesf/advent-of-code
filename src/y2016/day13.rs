...
fn part_a(designer: usize, dest: (usize, usize)) -> usize {
    ...
    stack.push(Pos { x: 1, y: 1, steps: 0, a_star: (dest_y as i32 - 1).abs() + (dest_x as i32 - 1).abs() });

    while let Some(pos) = stack.pop() {
        if !visited.insert((pos.x, pos.y)) { continue }
        if is_wall((pos.x, pos.y), designer) { continue }
        if pos.x == dest_x && pos.y == dest_y { return pos.steps; }

        if pos.x > 0 { stack.push(Pos { x: pos.x - 1, y: pos.y, steps: pos.steps + 1,
            a_star: manhattan_dist((pos.y, pos.x - 1), (dest_y, dest_x)) }) }
        stack.push(Pos { x: pos.x + 1, y: pos.y, steps: pos.steps + 1,
            a_star: manhattan_dist((pos.y, pos.x + 1), (dest_y, dest_x)) });
        if pos.y > 0 { stack.push(Pos { x: pos.x, y: pos.y - 1, steps: pos.steps + 1,
            a_star: manhattan_dist((pos.y - 1, pos.x), (dest_y, dest_x)) }) }
        stack.push(Pos { x: pos.x, y: pos.y + 1, steps: pos.steps + 1,
            a_star: manhattan_dist((pos.y + 1, pos.x), (dest_y, dest_x)) })
    }
...
fn part_b(designer: usize) -> usize {
    ...
    stack.push_back(Pos { x: 1, y: 1, steps: 0, a_star: 0 });

    while let Some(pos) = stack.pop_front() {
        if is_wall((pos.x, pos.y), designer) { continue }
        if pos.steps > 50 { continue }
        if !visited.insert((pos.x, pos.y)) { continue }

        if pos.x > 0 { stack.push_back(Pos { x: pos.x - 1, y: pos.y, steps: pos.steps + 1, a_star: 0 }) }
        stack.push_back(Pos { x: pos.x + 1, y: pos.y, steps: pos.steps + 1, a_star: 0 });
        if pos.y > 0 { stack.push_back(Pos { x: pos.x, y: pos.y - 1, steps: pos.steps + 1, a_star: 0 }) }
        stack.push_back(Pos { x: pos.x, y: pos.y + 1, steps: pos.steps + 1, a_star: 0 })
    }
