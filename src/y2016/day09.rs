...
pub(crate) fn day09() {
    println!(
        "{}",
        part_a(fs::read_to_string("input/2016/day09/input.txt").unwrap())
    );
    println!(
        "{}",
        part_b(fs::read_to_string("input/2016/day09/input.txt").unwrap())
    );
}
...
fn decompressed_size_v2(s: &str) -> usize {
    ...
    while i < s.len() {
        if chars[i] == '(' {
            ...
            in_bracket = false;
            let (l, r) = s[bracket_start..i].split_once("x").unwrap();
            size += decompressed_size_v2(
                s[(i + 1)..(i + 1 + l.parse::<usize>().unwrap())]
                    .to_string()
                    .as_str(),
...
mod day09_tests {
    ...
    fn decompressed_size_v2_works() {
        ...
        assert_eq!(
            241920,
            decompressed_size_v2("(27x12)(20x12)(13x14)(7x10)(1x12)A")
        );
        assert_eq!(
            445,
            decompressed_size_v2("(25x3)(3x3)ABC(2x3)XY(5x2)PQRSTX(18x9)(3x2)TWO(5x7)SEVEN")
        );
    }
