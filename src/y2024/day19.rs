...
fn is_possible(pattern: &Vec<char>, pos: usize, towels: &Vec<Vec<char>>) -> bool {
    if pos >= pattern.len() { return true; }
    towels.iter().any(|towel| {
        for i in 0..towel.len() {
            if pos + i >= pattern.len() { return false; }
            if pattern[pos + i] != towel[i] { return false; }
        }
...
fn count_possible_ways(pattern: &Vec<char>, pos: usize, towels: &Vec<Vec<char>>, cache: &mut HashMap<(Vec<char>, usize), usize>) -> usize {
    if pos >= pattern.len() { return 1; }
    ...
    towels.iter().for_each(|towel| {
        for i in 0..towel.len() {
            if pos + i >= pattern.len() { return; }
            if pattern[pos + i] != towel[i] { return; }
        }
