...
pub fn day20(print: fn(usize)) {
    let (part_a, part_b) = solve(
        fs::read_to_string("input/2024/day20/input.txt").unwrap(),
        100,
    );
    print(part_a);
    print(part_b);
        ...
        .map(|p| {
            ...
            for x in -20..=20_i32 {
                for y in -20..=20_i32 {
                    ...
                    if distances[*p] < target_saving {
                        continue;
                    } // No point in cheating if we are close to the E
                    let jump = P {
                        x: p.x + x,
                        y: p.y + y,
                    };
                    if !map.contains(&jump) {
                        continue;
                    }
                    if distances[*p] <= distances[jump] {
                        continue;
                    } // No point in cheating to a point that is furter from E
                    let saving =
                        distances[*p] - distances[jump] - p.manhattan_distance(&jump) as usize;
...
mod day20_tests {
    ...
    fn test_works() {
        assert_eq!(
            (5, 1449),
            solve(fs::read_to_string("input/2024/day20/test.txt").unwrap(), 20)
        );
    }
