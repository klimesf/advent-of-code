...
pub(crate) fn day07() {
    println!("{}", part_a(fs::read_to_string("input/2024/day07/input.txt").unwrap()));
    println!("{}", part_b(fs::read_to_string("input/2024/day07/input.txt").unwrap()));
}
...
fn part_b(input: String) -> usize {
    input.lines().map(|line| {
        ...
        while !stack.is_empty() {
            ...
            let mut prev_sum = digits[pos];
            while prev_sum > 0 {
                multiplier *= 10;
                prev_sum = prev_sum / 10;
            }
            stack.push((sum * multiplier + digits[pos], pos + 1));
        }
        0
    }).sum()
}
