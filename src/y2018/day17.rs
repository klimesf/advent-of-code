...
fn rec_floodfil(x: i32, y: i32, max_y: i32, water: &mut HashMap<(i32, i32), bool>, clay: &HashSet<(i32, i32)>) -> bool {
    ...
    if !clay.contains(&(x, y + 1)) {
        ...
        if !og_dripping && (*water.get(&(x - 1, y + 1)).unwrap_or(&true) == false || clay.contains(&(x - 1, y + 1))) && !clay.contains(&(x - 1, y)) {
        ...
        if dripping_right { // Correct the dripping level if one side is dripping
            let mut pos = (x, y);
        ...
        if dripping_left { // Correct the dripping level if one side is dripping
            let mut pos = (x, y);
            while !clay.contains(&pos) && water.contains_key(&pos) {
                water.insert(pos, true);
                pos = (pos.0 + 1, pos.1);
