use crate::utils::toolbox::gcd_64;
...
pub(crate) fn day21() {
    let input = fs::read_to_string("input/2022/day21/input.txt").unwrap();
    let monkeys: HashMap<&str, Job> = input
        ...
        .map(|line| {
            ...
            if job.contains("+") {
                ...
                (
                    name,
                    Job::OPERATION(left.to_string(), right.to_string(), Operation::PLUS),
                )
            } else if job.contains("-") {
                let (left, right) = job.split_once(" - ").unwrap();
                (
                    name,
                    Job::OPERATION(left.to_string(), right.to_string(), Operation::MINUS),
                )
            } else if job.contains("*") {
                let (left, right) = job.split_once(" * ").unwrap();
                (
                    name,
                    Job::OPERATION(left.to_string(), right.to_string(), Operation::MULTIPLY),
                )
            } else if job.contains("/") {
                let (left, right) = job.split_once(" / ").unwrap();
                (
                    name,
                    Job::OPERATION(left.to_string(), right.to_string(), Operation::DIVIDE),
                )
            } else {
            ...
            match job {
                ...
                Job::OPERATION(left, right, operation) => {
                    ...
                    if ans.contains_key(left) && ans.contains_key(right) {
                        match operation {
                            Operation::PLUS => {
                                ans.insert(name, ans.get(left).unwrap() + ans.get(right).unwrap())
                            }
                            Operation::MINUS => {
                                ans.insert(name, ans.get(left).unwrap() - ans.get(right).unwrap())
                            }
...
fn solve(monkey: &str, monkeys: &HashMap<&str, Job>) -> (Fraction, Fraction) {
    // -> (x, constant, multiply other side)
...
fn subtract_polynome(left: Polynome, right: Polynome) -> Polynome {
    (
        subtract_fraction(left.0, right.0),
        subtract_fraction(left.1, right.1),
    )
}
...
fn multiply_polynome(left: Polynome, right: Polynome) -> Polynome {
    if left.0 != ZERO && right.0 != ZERO {
        ...
        (
            multiply_fraction(left.0, right.1),
            multiply_fraction(left.1, right.1),
        )
    } else if right.0 != ZERO {
        (
            multiply_fraction(right.0, left.1),
            multiply_fraction(right.1, left.1),
        )
    } else {
...
fn divide_polynome(left: Polynome, right: Polynome) -> Polynome {
    if right.0 != ZERO {
        ...
        (
            divide_fraction(left.0, right.1),
            divide_fraction(left.1, right.1),
        )
    }
}
...
#[cfg(test)]
mod day17_tests {
    use crate::y2022::day21::{
        add_polynome, collapse, divide_fraction, divide_polynome, multiply_fraction,
        multiply_polynome, subtract_polynome, ZERO,
    };
    ...
    fn subtract_polynome_works() {
        assert_eq!(
            (ZERO, ZERO),
            subtract_polynome(((1, 1), ZERO), ((1, 1), ZERO))
        );
    }
    ...
    fn multiply_polynome_works() {
        assert_eq!(
            ((2, 1), (2, 1)),
            multiply_polynome(((1, 1), (1, 1)), (ZERO, (2, 1)))
        );
        assert_eq!(
            ((2, 1), (2, 1)),
            multiply_polynome((ZERO, (2, 1)), ((1, 1), (1, 1)))
        );
    }
    ...
    fn divide_polynome_works() {
        assert_eq!(
            ((1, 2), (1, 2)),
            divide_polynome(((1, 1), (1, 1)), (ZERO, (2, 1)))
        );
        assert_eq!(
            (ZERO, (1, 1)),
            divide_polynome((ZERO, (2, 1)), (ZERO, (2, 1)))
        );
    }
