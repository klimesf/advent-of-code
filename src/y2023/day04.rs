...
fn part_a(cards: String) -> i32 {
    cards.lines().map(|line| {
        ...
        let winning: Vec<i32> = winning_s.split_whitespace().map(|n| n.parse::<i32>().unwrap()).collect();

        let matches: Vec<i32> = right.split_whitespace()
            .map(|n| n.parse::<i32>().unwrap())
            .filter(|n| winning.contains(n))
            .collect();

        if matches.len() < 1 { return 0 }
...
fn part_b(cards: String) -> usize {
    ...
    cards.lines().enumerate().map(|(i, line)| {
        let mult = *ctr.entry(i).or_insert(1);
        let (_, numbers) = line.split_once(": ").unwrap();

        let (winning_s, right) = numbers.split_once(" | ").unwrap();

        let winning: Vec<usize> = winning_s.split_whitespace().map(|n| n.parse::<usize>().unwrap()).collect();

        let matches: Vec<usize> = right.split_whitespace()
