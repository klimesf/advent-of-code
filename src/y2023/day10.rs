...
fn solve(input: String) -> (usize, usize) {
    ...
    while !stack.is_empty() {
        ...
        let (r, c) = pos;
        if double_map[r][c] != '.' && distances.contains_key(&(r / 2, c / 2)) { continue; }
        ...
        if r < double_map.len() - 1 { stack.push((r + 1, c)) }
        if c > 0 { stack.push((r, c - 1)) }
        if c < double_map[r].len() - 1 { stack.push((r, c + 1)) }
    }
