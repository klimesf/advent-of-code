...
fn two_subsets_sum(items: Vec<usize>, target: usize) -> bool {
    ...
    while let Some((i, sum_b, sum_c, remaining)) = stack.pop() {
        if sum_b == target && sum_c == target && i == items.len() {
            return true;
        }
        if sum_b > target || sum_c > target {
            continue;
        }
        if i >= items.len() {
            continue;
...
fn three_subsets_sum(items: Vec<usize>, target: usize) -> bool {
    ...
    while let Some((i, sum_b, sum_c, sum_d, remaining)) = stack.pop() {
        if sum_b == target && sum_c == target && sum_d == target && i == items.len() {
            return true;
        }
        if sum_b > target || sum_c > target || sum_d > target {
            continue;
        }
        if i >= items.len() {
            continue;
