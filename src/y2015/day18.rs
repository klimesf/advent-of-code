...
fn part_a(input: String, steps: usize) -> usize {
    let mut lights: Vec<Vec<char>> = input.lines().map(|line| {
        line.chars().collect()
    }).collect();
            ...
            for c in 0..=size {
                ...
                for dr in [-1, 0, 1] {
                    for dc in [-1, 0, 1] {
                        ...
                        if r as i32 + dr < 0 || r as i32 + dr > size as i32 { continue }
                        if c as i32 + dc < 0 || c as i32 + dc > size as i32 { continue }
                        if lights[(r as i32 + dr) as usize][(c as i32 + dc) as usize] == '#' { neighbor_count += 1 }
                    }
                }

                if lights[r][c] == '#' {
                    new_lights[r][c] = if neighbor_count == 2 || neighbor_count == 3 { '#' } else { '.' }
                } else {
...
fn part_b(input: String, steps: usize) -> usize {
    ...
    for _ in 0..steps {
        ...
        for r in 0..=size {
            for c in 0..=size {
                if (r == 0 && c == 0) || (r == 0 && c == size)
                    || (r == size && c == 0) || (r == size && c == size) {
                    new_lights[r][c] = '#';
                    continue
                }
